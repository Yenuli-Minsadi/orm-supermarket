...
//            List<Customer> list = query.list();
    ...
    public List<Customer> search(String text) throws SQLException {
        ...
        Session session = factoryConfiguration.getSession();
        try {
            Query<Customer> query = session.createQuery("FROM Customer c WHERE c.id OR" +
                                                        "c.name:text OR c.nic LIKE :text OR" +
                                                        "c.email LIKE :text OR c.phone LIKE :text", Customer.class);
